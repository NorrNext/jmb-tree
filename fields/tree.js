...
function dtmlHTMLLoaderObject(funcObject, dhtmlObject) {
	this.HTMLDoc = "";
...
dtmlHTMLLoaderObject.prototype.waitLoadFunction = function (dhtmlObject) {
	dhtmlObject.onloadAction(dhtmlObject.mainObject);
};


dtmlHTMLLoaderObject.prototype.loadHTMLString = function (HTMLString) {
	this.onloadAction(this.mainObject);
}
dtmlHTMLLoaderObject.prototype.loadHTML = function (filePath) {
	this.HTMLDoc.onload = new this.waitLoadFunction(this);
};
...
function dhtmlXTreeObject(htmlObject, width, height, rootId) {
	...
	this.HTMLLoader = new dtmlHTMLLoaderObject(this._parseHTMLTree, this);
...
dhtmlXTreeObject.prototype.loadHTMLString = function (HTMLString) {
	this.HTMLLoader.loadHTMLString(HTMLString);
};

dhtmlXTreeObject.prototype.loadHTML = function (file) {
	this.HTMLLoader.loadHTML(file);
};
...
dhtmlXTreeObject.prototype._attachChildNode = function (parentObject, itemId, itemText, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode) {
	...
	if (((parentObject.HTMLload == 0) && (this.HTMLsource)) && (!this.HTMLloadingWarning)) {
		parentObject.HTMLload = 1;
		this.loadHTML(this.HTMLsource + getUrlSymbol(this.HTMLsource) + "itemId=" + escape(parentObject.itemId));
	}
	...
	if (optionStr) {
		...
		for (var i = 0; i < tempStr.length; i++) {
			switch (tempStr[i]) {
				case "TOP":
					if (parentObject.childsCount > 1)beforeNode = parentObject.htmlNode.childNodes[0].childNodes[1].nodem.previousSibling;
					break;
			}
		}
		;
	...
	if (this.HTMLsource)if ((childs) && (childs != 0))Nodes[Count].HTMLload = 0; else Nodes[Count].HTMLload = 1;
...
dhtmlXTreeObject.prototype._parseHTMLTree = function (dhtmlObject, node, parentId, level) {
	dhtmlObject.HTMLloadingWarning = 1;
	var nodeAskingCall = "";
	if (!node) {
		node = document.getElementById(dhtmlObject.rootId);
	...
	for (var i = 0; i < node.childNodes.length; i++) {
		...
		if ((node.childNodes[i].nodeName == "DIV")) {
			...
			var zcall = dhtmlObject._parseHTMLTree(dhtmlObject, node.childNodes[i], cId, 1);
			if (zcall != "")nodeAskingCall = zcall;
		}
		else if ((node.childNodes[i].nodeType == 1) && (node.childNodes[i].tagName == "userdata")) {
	...
	if (!level) {
		dhtmlObject.lastLoadedHTMLId = parentId;
		dhtmlObject._redrawFrom(dhtmlObject);
		dhtmlObject.HTMLloadingWarning = 0;
...
dhtmlXTreeObject.prototype._redrawFrom = function (dhtmlObject, itemObject) {
	if (!itemObject) {
		var tempx = dhtmlObject._globalIdStorageFind(dhtmlObject.lastLoadedHTMLId);
		dhtmlObject.lastLoadedHTMLId = -1;
		if (!tempx)return 0;
	}
...
dhtmlXTreeObject.prototype._correctPlus = function (itemObject) {
	...
	if ((this.HTMLsource) && (!itemObject.HTMLload)) {
...
dhtmlXTreeObject.prototype._HideShow = function (itemObject, mode) {
	if (((this.HTMLsource) && (!itemObject.HTMLload)) && (!mode)) {
		itemObject.HTMLload = 1;
		this.loadHTML(this.HTMLsource + getUrlSymbol(this.HTMLsource) + "id=" + escape(itemObject.id));
		return;
	}
	;
...
dhtmlXTreeObject.prototype.onRowSelect = function (e, htmlObject, mode) {
	...
	if (!mode) {
		if (htmlObject.parentObject.actionHandler)htmlObject.parentObject.actionHandler(htmlObject.parentObject.id);
	}
};
...
dhtmlXTreeObject.prototype._correctCheckStates = function (dhtmlObject) {
	...
	for (var i = 1; i < act.length; i++)
		if (act[i].nodem.checkstate == 0) {
			flag1 = 1;
			flag2 = 1;
		} //SHERZA: добавила flag2=1; , чтобы при убирании галок с дочерних элементов не убиралась галка с родительских
...
dhtmlXTreeObject.prototype.setHTMLAutoLoading = function (filePath) {
	this.HTMLsource = filePath;
};
...
dhtmlXTreeObject.prototype._moveNode = function (itemObject, targetObject) {
	...
	if ((targetObject.HTMLload == 0) && (this.HTMLsource)) {
		targetObject.HTMLload = 1;
		this.loadHTML(this.HTMLsource + getUrlSymbol(this.HTMLsource) + "id=" + escape(targetObject.id));
	}
...
dhtmlXTreeObject.prototype.hasChildren = function (itemId) {
	...
	if (!temp)return 0;
		...
		if ((this.HTMLsource) && (!temp.HTMLload))return true;
		else
...
dhtmlXTreeObject.prototype.refreshItem = function (itemId) {
	...
	this.loadHTML(this.HTMLsource + getUrlSymbol(this.HTMLsource) + "id=" + escape(itemId));
};
...
//SHERZA: было - (mode)&&(htmlNode.checkstate=1)
	if (((mode) && (htmlNode.checkstate > 0)) || ((!mode) && (htmlNode.checkstate > 0))) {
